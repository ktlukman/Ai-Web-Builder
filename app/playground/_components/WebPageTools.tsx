...
const downloadCode = () => {
  ...
  return (
    <div className='p-2 shadow rounded-xl w-full flex items-center justify-between'>
        ...
        <Button className={`${selectedScreenSize === 'web' ? 'border border-primary' : ''}`} onClick={()=>setSelectedScreenSize('web')} variant={`ghost`}><Monitor /></Button>
        <Button className={`${selectedScreenSize === 'mobile' ? 'border border-primary' : ''}`} onClick={()=>setSelectedScreenSize('mobile')} variant={`ghost`}><TabletSmartphone /></Button>
        </div>

        <div className='flex gap-2'>
            <Button variant={`outline`} onClick={()=>ViewInNewTab()}> View <SquareArrowOutUpRight /></Button>
            <ViewCodeBlock code={finalCode}><Button>Code <Code2Icon /></Button></ViewCodeBlock>
